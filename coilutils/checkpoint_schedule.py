...
def get_latest_evaluated_checkpoint_2(filename=None):
    ...
    data_matrix = np.loadtxt(open(filename, "rb"),
...
def get_next_checkpoint_2(checkpoint_schedule, filename=None):

    ltst_check = get_latest_evaluated_checkpoint_2(filename)
    if ltst_check is None:
        return checkpoint_schedule[0]
