...
from cexp.benchmark import check_benchmarked_episodes_metric
...
def get_episode_number(json_filename, agent_checkpoint_name):
    """ Return the number of different benchmarks that were benchmarked"""

    #TODO take repetition into consideration

    summary_benchmark = check_benchmarked_episodes_metric(json_filename, agent_checkpoint_name)


    if not summary_benchmark:
        return 0

    return len(summary_benchmark['result'])

def get_number_episodes_completed(json_filename, agent_checkpoint_name):
    ...
    return sum(summary_benchmark['result'])
...
def get_latest_checkpoint_drive(package_name):
    # TODO update this to get the folder directly
    # add csv to the file name

    control_filename = package_name + '.csv'
