...
class ETEAgent(object):

    def __init__(self, path_to_config_file):

        yaml_conf, checkpoint_number, agent_name, _ = checkpoint_parse_configuration_file(path_to_config_file)

        # Take the checkpoint name and load it
        print (" FIle to load", os.path.join('/', os.path.join(*os.path.realpath(__file__).split('/')[:-2]),
                                              '_logs',
        ...
        # do the merge here
        print ("yaml to merge", os.path.join('/', os.path.join(*os.path.realpath(__file__).split('/')[:-2]),
                                     yaml_conf))
    ...
    def get_state(self, exp_list):
        ...
        input_data = {'rgb_central': exp.get_sensor_data()['rgb_central'],
                 'speed': exp.get_sensor_data()['can_bus'][1]['speed'],
    ...
    def step(self, input_data):
        ...
        norm_speed = input_data['speed'] / g_conf.SPEED_FACTOR
        norm_speed = torch.cuda.FloatTensor([norm_speed]).unsqueeze(0)
        directions = encode_directions(directions)
        directions_tensor = torch.from_numpy(np.asarray([directions])).float().cuda()
        # Compute the forward pass processing the sensors got from CARLA.

        model_outputs = self._model.forward_action(self._process_sensors(input_data['rgb_central'][1]),
                                                   norm_speed,
                                                   directions_tensor)
