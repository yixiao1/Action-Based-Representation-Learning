...
class CoILAgent(object):
    ...
    def __init__(self, checkpoint, town_name, carla_version='0.84', vae_params = None):
        ...
        if g_conf.VAE_MODEL_CONFIGURATION != {}:
            ...
            VAE_checkpoint = torch.load(
                os.path.join('_logs', vae_params['vae_folder'], vae_params['vae_exp'], 'checkpoints',
                             str(vae_params['vae_checkpoint']) + '.pth'))
            print("VAE model ", str(vae_params['vae_checkpoint']), " already loaded from ",
                  os.path.join('_logs', vae_params['vae_folder'], vae_params['vae_exp'], 'checkpoints'))
    ...
    def run_step(self, measurements, sensor_data, directions, target):
        ...
        if g_conf.VAE_MODEL_CONFIGURATION != {}:
            ...
            model_outputs = self._model.forward_branch(z, norm_speed,directions_tensor)
