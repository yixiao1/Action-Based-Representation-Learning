...
class ACTION_PREDICTION(nn.Module):
    ...
    def forward(self, x, m, c, a):
        ...
        Args:
            ...
            a: the action (steer, throttle, brake) ground truth at time t_prev.
        ...
        # NOTE The lateral cameras might be tricky.
        # TODO when using the lateral cameras
        # lateral cameral at time t after action becomes central camera time t+1


        # the forward part of model
        # To input image to model
        x_t, inter = self.encode_conv(x[0])  # frame before the action
        ...
        loss = torch.abs(out - a)
        loss = torch.sum(loss[:, 0] * g_conf.VARIABLE_WEIGHT['Steer'] \
                      + loss[:, 1] * g_conf.VARIABLE_WEIGHT['Gas'] \
                      + loss[:, 2] * g_conf.VARIABLE_WEIGHT['Brake'])/ g_conf.BATCH_SIZE
...
class ETE_inverse_model(nn.Module):
    def __init__(self, params):
        ...
        if 'res' in params['perception']:  # pre defined residual networks
            resnet_module = importlib.import_module('network.models.building_blocks.resnet')
            resnet_module = getattr(resnet_module, params['perception']['res']['name'])
            self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                             num_classes=params['perception']['res']['num_classes'])
    ...
    def forward(self, x,  m,  c,  a):
        ...
        inverse_loss = torch.abs(inverse_out - a)
        inverse_loss = torch.sum(inverse_loss[:, 0] * g_conf.VARIABLE_WEIGHT['Steer'] \
                         + inverse_loss[:, 1] * g_conf.VARIABLE_WEIGHT['Gas'] \
                         + inverse_loss[:, 2] * g_conf.VARIABLE_WEIGHT['Brake']) / g_conf.BATCH_SIZE
...
class STDIM(nn.Module):
    ...
    def forward(self, x, m, c):
        ...
        x_t_prev, inter_prev = self.encode_conv(x[0])  # frame before the action
        x_t, inter = self.encode_conv(x[1])  # frame after the action

        """ ###### APPLY THE MEASUREMENT MODULE """
        m_t_prev = self.measurements(m[0])
        m_t = self.measurements(m[1])
        """ ###### APPLY THE Command MODULE  that can also be output of another network"""
        c_t_prev = self.command(c[0])
        c_t = self.command(c[1])
        """ Join measurements and perception"""
        f_t_global = self.join_obs(x_t, m_t, c_t)
    ...
    def forward_encoder(self, x, m, c):
        ...
        # the forward part of model
        # To input image to model
        x, inter = self.encode_conv(x)  # encoding with the resnet.
...
class ForwardInverse(nn.Module):

    # The normal forward model that uses the ground truth and
    def __init__(self, params):
        super(ForwardInverse, self).__init__()
        self.params = params
    ...
    def forward(self, x, m, c, a):
        ...
        Returns:
        """
        # Encode f_t and f_t+1
        f_t, x_t, inter = self.encoder(x[0], m[0], c[0])
        f_ti, x_ti, inter_2 = self.encoder(x[1], m[1], c[1])

        """ We expand the action representation"""
        # Perform the action space expansion, the action at time t input expansion
        a_t = self.action(a)
        # merge with F_t to get the possible next state
        f_ti_pred = self.join_forward(f_t, a_t)
        # Get the l1 loss with the true f_t+1
        loss_forward = F.l1_loss(f_ti_pred, f_ti)
        # Join the predicted f_ti_pred with the first state.
        j_f = self.join_inverse(f_t, f_ti)
        # for the predicted f_t_pred
        inverse_out = self.inverse(j_f)
        ...
        loss = loss_forward * g_conf.LOSSES_WEIGHTS['forward'] + \
                inverse_loss * g_conf.LOSSES_WEIGHTS['inverse']

        return loss, loss_forward, inverse_loss
    ...
    def forward_encoder(self, x, m, c):
        ...
        f_t, x_t, inter = self.encoder(x, m, c)
        return f_t, inter



class FIMBC(nn.Module):

    # The normal forward model that uses the ground truth and
    def __init__(self, params):
        super(FIMBC, self).__init__()
        self.params = params

        self.encoder = Encoder(params)

        self.action_feat = FC(params={'neurons': params['join']['fc']['neurons'] +
                                                params['action']['fc']['neurons'],
    ...
    def forward(self, x, m, c, a, a_c):
        ...
        a_feat = self.action_feat(f_t)
        a_pred = self.action(a_feat)

        N = f_t.size(0)
        loss_bc_action = torch.abs(a_pred - a)
        loss_bc_action = loss_bc_action[:, 0] * g_conf.VARIABLE_WEIGHT['Steer'] \
                         + loss_bc_action[:, 1] * g_conf.VARIABLE_WEIGHT['Gas'] \
                         + loss_bc_action[:, 2] * g_conf.VARIABLE_WEIGHT['Brake']

        loss_bc_speed = torch.abs(self.speed_branch(x_t) - m[0])

        loss_bc = torch.sum(loss_bc_action) * g_conf.BRANCH_LOSS_WEIGHT[0]/N \
                  + torch.sum(loss_bc_speed) * g_conf.BRANCH_LOSS_WEIGHT[1]/N

        # merge with F_t to get the possible next state
        f_ti_pred = self.join_forward(f_t, a_feat)
        # Get the l1 loss with the true f_t+1
        loss_forward = F.l1_loss(f_ti_pred, f_ti)
        # Join the predicted f_ti_pred with the first state.
        j_f = self.join_inverse(f_t, f_ti)
        ...
        loss1 = F.cross_entropy(inverse_out[:, 0:x_pos], a_c[:, 0].long(),
                                weight=torch.FloatTensor(
                                    g_conf.ACTION_VARIABLE_WEIGHT['steer']).cuda())
        loss2 = F.cross_entropy(inverse_out[:, x_pos:x_pos + y_pos], a_c[:, 1].long(),
                                weight=torch.FloatTensor(
                                    g_conf.ACTION_VARIABLE_WEIGHT['throttle']).cuda())
        loss3 = F.cross_entropy(inverse_out[:, x_pos + y_pos:x_pos + y_pos + z_pos],
                                a_c[:, 2].long(),
                                weight=torch.FloatTensor(
                                    g_conf.ACTION_VARIABLE_WEIGHT['brake']).cuda())
        inverse_loss = loss1 + loss2 + loss3

        loss = loss_bc * g_conf.LOSSES_WEIGHTS['bc'] + \
               loss_forward * g_conf.LOSSES_WEIGHTS['forward'] + \
               inverse_loss * g_conf.LOSSES_WEIGHTS['inverse']
...
class FIMBC2(nn.Module):

    # The normal forward model that uses the ground truth and
    def __init__(self, params):
        super(FIMBC2, self).__init__()
        self.params = params
        ...
        self.forward_model = FC(params={'neurons': [3] +
                                            params['join']['fc']['neurons'] +
                                            params['action']['fc']['neurons'],
                                 'dropouts': params['action']['fc']['dropouts'] + [0.0],
                                 'end_layer': False})
