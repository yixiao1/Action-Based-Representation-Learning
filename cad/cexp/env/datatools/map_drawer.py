...
def draw_point_data_relative_angle(datapoint, color=None, alpha=None):
    """
    We draw in a certain position at the map
    :param position:
    :param color:
    ...
    if color is None:
        ...
        result_color = get_color_relative_angle(datapoint['measurements']['ERROR_relative_angle'])
...
def get_color_relative_angle(ERROR_relative_angle):
    """
    Based on the scenario we paint the trajectory with a given color.
    :param scenario:
    :return:
    ...
    if abs(ERROR_relative_angle) <= 0.09:      # 5 degree
        return COLOR_CHOCOLATE_0
    elif abs(ERROR_relative_angle) <= 0.18:    # 10 degree
        return COLOR_ORANGE_0
    elif abs(ERROR_relative_angle) <= 0.35:    # 20 degree
        return COLOR_ORANGE_2
    elif abs(ERROR_relative_angle) <= 0.52:    # 30 degree
        return COLOR_SCARLET_RED_0
    elif abs(ERROR_relative_angle) <= 0.87:    # 50 degree
        return COLOR_SCARLET_RED_1
    else:
        return COLOR_SCARLET_RED_2
    ...
    for exp in env_data:
        ...
        for batch in exp[0]:
            ...
            while step < len(batch[0]):
                ...
                #    first_time = False
                #else:
                draw_point_data(batch[0][step], color=COLOR_CHOCOLATE_0, direct_read=direct_read)
                step += step_size

    fig.savefig(os.path.join(directory, env_name + '_trajectory.png'),
                orientation='landscape', bbox_inches='tight', dpi=1200)

def draw_relative_angle_error(directory, env_data, env_name, world, route, step_size=3, direct_read=False):
    ...
    for exp in env_data:
        ...
        for batch in exp[0]:
            ...
            while step < len(batch[0]):
                ...
                #    first_time = False
                #else:
                draw_point_data_relative_angle(batch[0][step])
                step += step_size

    fig.savefig(os.path.join(directory, env_name + '_relative_angle_error_trajectory.png'),
                orientation='landscape', bbox_inches='tight', dpi=1200)
