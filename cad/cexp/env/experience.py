...
class Experience(object):
    ...
    def tick_world(self):
        ...
        _, self._directions = self._get_current_wp_direction(self._ego_actor.get_transform().location,
                                                             self._route)

        self._forward_speed = get_forward_speed(self._ego_actor)

        if self._save_data:

            # HERE we may adapt the npc to stop dist_scenario3

            transform = self._ego_actor.get_transform()
            velocity = self._ego_actor.get_velocity()
            self._environment_data['exp_measurements'] = {
                'directions': self._directions,
                'forward_speed': self._forward_speed,
                'ego_actor':{"position": [transform.location.x, transform.location.y, transform.location.z],
    ...
    def save_experience(self, state=None):

        if self._save_data:
            self._sensor_interface.wait_sensors_written(self._writer)
            self._writer.save_experience(self.world, self._environment_data, state)
    ...
    def _load_world(self):
        ...
        while attempts < self.MAX_CONNECTION_ATTEMPTS:
            try:
                #print(self._client.get_client_version())
