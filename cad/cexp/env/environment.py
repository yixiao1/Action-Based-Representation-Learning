...
from cexp.env.datatools.map_drawer import draw_pedestrians, draw_opp_trajectories, \
                                          draw_trajectories, draw_relative_angle_error
...
class Environment(object):
    ...
    def get_data(self):
        ...
        full_episode_data_dict = parser.parse_environment(root_path, metadata_dict, agent_name=self._last_executing_agent)
    ...
    def step(self, control_vec, state=None):
        """
    ...
    def draw_relative_angle_error(self, directory):
        # Draw the trajectory made on the last instance of this enviroment (experience)
        if not os.path.exists(directory):
            os.mkdir(directory)
        draw_relative_angle_error(directory,
                          self.get_data(),
                          self._last_executing_agent + '_' + self._package_name + '_' + self._environment_name,
                          self._exp_list[0].world,
                          self._exp_list[0]._route,
